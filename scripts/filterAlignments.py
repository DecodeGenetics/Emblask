...
if __name__ == "__main__":
	...
	optional.add_argument('-o', '--out_bam', action='store', help='Output BAM filename without filtered out alignments', required=False)
	...
	optional.add_argument('-e', '--min_error_rate', action='store', help='Filter out any alignment with error rate larger than this ratio ("de" tag required in alignment records). 1.0 is no filtering.', type=float, default=1.0, required=False)
	optional.add_argument('-E', '--min_error_rate_boundary', action='store', help='Filter out alignments overlapping reference sequence boundary with error rate larger than this ratio ("de" tag required in alignment records). 1.0 is no filtering.', type=float, default=1.0, required=False)
	...
	out_bamf = pysam.AlignmentFile(args.out_bam, "wb", template=in_bamf, threads=args.threads) if args.out_bam != None else None
	...
	for rec in in_bamf.fetch(until_eof=args.keep_unmapped):
		...
		if rec.is_unmapped:
			...
			if not args.keep_unmapped: rm_umap_s.add(rec.query_name)
			elif out_bamf != None: out_bamf.write(rec)
		...
		# - Aligned length is less than minimum required but MAPQ is 60 (the maximum)
			...
			if (args.min_error_rate < 1.0) or (args.min_error_rate_boundary < 1.0):
			
				if not rec.has_tag("de"): sys.exit("Record found without the de tag which is required for error rate filtering. Abort.")
				
				ref_len = in_bamf.get_reference_length(rec.reference_name)

				overlaps_start = (rec.reference_start == 0)
				overlaps_end = (rec.reference_end == ref_len)
				
				de_tag = float(rec.get_tag("de"))
				
				if overlaps_start or overlaps_end:
				
					if de_tag >= min(args.min_error_rate, args.min_error_rate_boundary): rm_map_s.add(rec.query_name)
					elif out_bamf != None: out_bamf.write(rec)
					
				elif de_tag >= args.min_error_rate: rm_map_s.add(rec.query_name)
				elif out_bamf != None: out_bamf.write(rec)
				
			elif out_bamf != None: out_bamf.write(rec) # No filtering on error rate

		else: rm_map_s.add(rec.query_name)

	in_bamf.close()
	
	if out_bamf != None: out_bamf.close()
	
	for query_name in rm_map_s: print(query_name)
	for query_name in rm_umap_s: print(query_name)