...
process filterONT {
	...
	script:
		// Ideally, a_cpus gets 66% of the allocated cpus (to a minimum of 1) and the other commands get the rest
		def a2b_cpu_ratio = 0.66
		// Any job other than A can never exceed 4 cpus
		def not_a_max_cpus = 4
...
process extractPairedIllumina {
	...
	script:
		// a_cpus gets 66% of the allocated cpus to a minimum of 1
		// b_cpus gets the rest of the allocated cpus to a minimum of 1
		// b_cpus cannot exceed 4 cpus, in which case a_cpus gets the rest to a minimum of 4
		def a2b_cpu_ratio = 0.66
		def b_cpu_max = 4
		...
		\${samtools} collate -@ ${a_cpus} -u -O -n 128 ${sr_bam_in} /tmp/collate | \${samtools} fastq -@ ${b_cpus} -o sr.${task.process}.fq.gz -;
		"""
}
...
process splitPairedIllumina {
	...
	script:
		...
		\${pigz} -p ${a_cpus} -d -c ${sr_fq} | split -l 80000000 -d - sr.${task.process}.fq.part_;
		"""
}
...
process mixHapAsm_filter {
	...
	script:
		...
		cat haplotig.low_het_var_count.list haplotig.unbalanced_phase.list | sort -u > haplotigs.list; comm -23 <(awk '{print \$1}' asm.bed | sort -k1,1) haplotigs.list > contig.collapsed.list
...
process prepLocalCorrection  {
	...
	script:
		...
		do
			...
			join ../lr.corrected.dict <(cut -f1 lr.corrected.\${ID_JOB}.bed | sort -u) | awk '{print \$1 \"\\t\" \$2}' > lr.corrected.\${ID_JOB}.dict
		done
		"""
}
...
process mergeLocalCorrection {
	...
	script:
		...
		def a_cpus_tmp = Math.max(1, Math.round(task.cpus * a2b_cpu_ratio) as int)
		def bc_cpus_tmp = Math.max(1, task.cpus - a_cpus_tmp)
		
		def b_cpus = Math.min(not_a_max_cpus,  Math.max(1, bc_cpus_tmp.intdiv(2)))
		def c_cpus = Math.min(not_a_max_cpus,  Math.max(1, bc_cpus_tmp - b_cpus))
		def a_cpus = Math.max(1, task.cpus - b_cpus - c_cpus)
		...
		\${samtools} view -@ ${a_cpus} -b -f 4 lr.asm.bam | \${samtools} bam2fq -n -@ ${b_cpus} - 2> /dev/null | \${pigz} -p ${c_cpus} -c >> lr.${task.process}.fastq.gz
...
process hapResAsm_filter {
	...
	script:
		...
		\${samtools} depth -@ ${a_cpus} -J -Q ${params.pipeline.min_mapq_strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.bam | \
...
process hapResAsm_polish1_1 {
	...
	script:
		def a_cpus_minus2 = Math.max(1, task.cpus - 2)
		def a_cpus_minus1 = Math.max(1, task.cpus - 1)
		...
		\${samtools} depth -@ ${a_cpus_minus2} -aa -J -G ${params.pipeline.samtools.depth.filter_supp_sec} lr.asm.bam | \
		...
		\${samtools} depth -@ ${a_cpus_minus2} -aa -J -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.bam | \
		awk 'BEGIN {FS=\"\\t\"; OFS=\"\\t\"; CONTIG=\"\"; SUM=0; COUNT=0;} \
		{if (\$1!=CONTIG) {if (CONTIG!=\"\") {MEAN=0; if (COUNT>=1) {MEAN=SUM/COUNT}; print CONTIG, MEAN}; CONTIG=\$1; SUM=0; COUNT=0}; SUM+=\$3; COUNT+=1} \
		...
		\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.filtered.bam | \
		...
		\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.${task.process}.bam | \
...
process hapResAsm_polish1_2 {
	...
	script:
		...
		awk '{if (\$4!=\"ContigEnd\"){print \$1}}' MARGIN_PHASED.phaseset.bed | cut -f1 | sort -u > MARGIN_PHASED.phaseset.inconsistent.list
		...
		\${samtools} index -@ ${task.cpus} lr.asm.${task.process}.splitPS.bam
		\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.${task.process}.splitPS.bam | \
		awk -v min_cov=\${COLLAPSED_COV_MIN} -v max_cov=\${COLLAPSED_COV_MAX} 'BEGIN {FS=\"\\t\"; OFS=\"\\t\"; CONTIG=\"\"; POS_S=-1; POS_E=-1} \
		{if ((\$3>=min_cov) && (\$3<=max_cov)) {if ((\$1==CONTIG) && (\$2==POS_E)) {POS_E+=1} else { if (POS_S!=-1) {print CONTIG, (POS_S-1), (POS_E-1)}; CONTIG=\$1; POS_S=\$2; POS_E=\$2+1;}}} \
...
process hapResAsm_polish2 {
	...
	script:
		...
		\${samtools} depth -@ ${a_cpus_minus1} -aa -J -Q 0 -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.bam | \
		...
		\${samtools} view -@ ${a_cpus_minus2} -F 2048 --tag de ref2ref.bam | awk '{if ((\$5==0) && (\$1!=\$3)) {for (i=6; i<=NF; i+=1){ if (substr(\$i,1,5)==\"de:f:\") \
		{ERR=substr(\$i,6,length(\$i)-5); if (ERR<=0.001) {print \$1}; break;}}}}' | sort -u > duplicates.list
...
process hapResAsmPolish_phase_varFilter {
	...
	script:
		...
		COV=\$(\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} -b lr.asm.hq.bed lr.asm.bam | \
		...
		\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.bam | \
		awk -v min_cov=\${COLLAPSED_COV_MIN} -v max_cov=\${COLLAPSED_COV_MAX} 'BEGIN {FS=\"\\t\"; OFS=\"\\t\"; CONTIG=\"\"; POS_S=-1; POS_E=-1} \
...
process hapResAsmPolish_phase_varPhasedFilter {
	...
	script:
		...
		\${samtools} depth -@ ${a_cpus_minus1} -a -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} -b lr.asm.hap.filt.bed lr.asm.hap.bam | \
		...
		cat lr.asm.hap.filt.cov <(\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.asm.hap.bam) | \
...
process hapResAsmPolish_extractPhased {
	...
	script:
		...
		python \${extract_ref_reads_py} -t ${a_cpus} -b h2.bam -v lr.hap.vcf.gz -p | \${pigz} -p ${b_cpus} -c > h2.hap.fastq.gz
		python \${extract_ref_reads_py} -t ${a_cpus} -b h1.bam -v lr.hap.vcf.gz -p --alt | \${pigz} -p ${b_cpus} -c >> h2.hap.fastq.gz
		python \${extract_ref_reads_py} -t ${a_cpus} -b h2.bam -v lr.hap.vcf.gz -p --alt | \${pigz} -p ${b_cpus} -c >> h1.hap.fastq.gz
...
process hapResAsmPolish_getCollapsedHom {
	...
	script:
		...
		// Half of the cores minus 1 for A
		def a_cpus_half_minus1 = Math.max(1, task.cpus.intdiv(2) - 1)
		// Half of the cores minus 2 for A
		def a_cpus_half_minus2 = Math.max(1, task.cpus.intdiv(2) - 2)
		// All the cores minus 1 for A
		def a_cpus_minus1 = Math.max(1, task.cpus - 1)
		// All the cores minus 2 for A
		def a_cpus_minus2 = Math.max(1, task.cpus - 2)
		...
		def b_cpus_minus1 = Math.max(1, b_cpus - 1)
		
		// 34% of the cores for all commands but A
		def bc_cpus_tmp = Math.max(1, task.cpus - a_cpus_tmp)
		// Half of the 34% of the cores but cannot exceed 4 cores for B
		def b_cpus2 = Math.min(not_a_max_cpus,  Math.max(1, bc_cpus_tmp.intdiv(2)))
		...
		<(\${samtools} depth -@ ${a_cpus_half_minus2} -aa -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} -b lr.hap.phased.bed lr.hap.unphased.bam | \
		...
		join <(\${samtools} depth -@ ${a_cpus_half_minus2} -aa -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} -b lr.hap.phased.bed lr.hap.alt.bam | \
		awk '{print \$1 \"\\t\" \$2 \"\\t\" \$3}' | sort -k1,1 -k2,2n) \
		<(\${samtools} depth -@ ${a_cpus_half_minus2} -aa -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} -b lr.hap.phased.bed lr.hap.unphased.bam | \
		awk '{print \$1 \"\\t\" \$2 \"\\t\" (\$3/2)}' | sort -k1,1 -k2,2n) | \
		tr ' ' '\t' | awk '{if (\$1!=CONTIG){if (CONTIG!=\"\") {MEAN=SUM/COUNT; SUMSQ=0; for (i=1; i<=COUNT; i++) {SUMSQ+=(SAMP[i]-MEAN)^2}; print CONTIG \"\\t\" MEAN \"\\t\" sqrt(SUMSQ/COUNT)}; \
		...
		cat lr.hap.phased.cov <(\${samtools} depth -@ ${a_cpus_minus2} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.hap.phasedSupp2prim.bam) | \
		...
		\${samtools} depth -@ ${a_cpus_minus1} -aa -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.hap.phased.bam | \
		...
		cat lr.hap.phased.cov <(\${samtools} depth -@ ${a_cpus_minus1} -J -Q ${params.pipeline.min_mapq.strict} -G ${params.pipeline.samtools.depth.filter_sec} lr.hap.phasedSupp2prim.bam) | \
		...
		\${pigz}  -d -c -p ${a_cpus} lr.hap.homozygous.fastq.gz | awk '{if (((NR-1)%8)<4){print \$0}}' | \${pigz} -p ${b_cpus_minus1} -c > h1.homozygous.fastq.gz
		\${pigz}  -d -c -p ${a_cpus} lr.hap.homozygous.fastq.gz | awk '{if (((NR-1)%8)>=4){print \$0}}' | \${pigz} -p ${b_cpus_minus1} -c > h2.homozygous.fastq.gz
		\${pigz}  -d -c -p ${a_cpus_minus1} lr.hap.homozygous.fastq.gz | awk '{if (NR%4==1){print substr(\$0,2,length(\$0)-1)}}' > lr.hap.homozygous.list
...
process hapResAsmPolish_polishDualAsm_1 {
	...
	script:
		...
		do
			...
			sort -u > h\${HAP_A}_map2_h\${HAP_B}.high_sim.list
			...
			\${samtools} depth -@ ${a_cpus_minus1} -aa -J -Q ${params.pipeline.min_mapq.strict} -G 1540 -b h\${HAP_A}_map2_h\${HAP_B}.high_sim.bed lr.h\${HAP_A}.bam | \
...
workflow {
	...
	// Split a paired Illumina FASTQ into chunks
	sr_chunks_fq = splitPairedIllumina(proband_sr_fq).flatten()

	// Filter input corrected long reads based on corrected base quality
	lr_filt_fq = filterONT(proband_lr_bam, proband_lr_fq)
	// Mixed haplotype assembly from corrected filtered long reads
	mixhap_asm = mixHapAsm(lr_filt_fq)

	// Remap filtered corrected long reads to mix haplotype assembly
	lr_map2mixhap_asm = mapLRtoAsm_1(lr_filt_fq, mixhap_asm)
	...
	// Filter haplotigs from mixed haplotype assembly
	filt_mixhap_asm = mixHapAsm_filter(filt_mixhap_asm_phase.hap_bam, mixhap_asm, filt_mixhap_asm_call, lr_map2mixhap_asm.cov)

	sr_chunks_fq.combine(filt_mixhap_asm).set { sr_chunks_filt_mixhap_asm }

	// Map the splitted Illumina chunks to the filtered mixed haplotype assembly
	sr_filt_mixhap_asm_chunks_bam = mapPairedIllumina_correct(sr_chunks_filt_mixhap_asm, true)
	...
	sr_filt_mixhap_asm_merge_bam = mergePairedIlluminaBAM_correct(sr_filt_mixhap_asm_chunks_bam_split.bam.collect(), sr_filt_mixhap_asm_chunks_bam_split.bam_bai.collect())

	// Map the filtered long reads to the filtered mixed haplotype assembly
	lr_filt_map2mixhap_asm = mapLRtoAsm_2(lr_filt_fq, filt_mixhap_asm)

	// Prep filtered ONT reads for local correction with short reads
	local_lr_corr_chunks_prep = prepLocalCorrection(lr_filt_map2mixhap_asm.bam, sr_filt_mixhap_asm_merge_bam)

	// This is ugly and we must find a workaround at some point
	local_lr_corr_chunks_prep.flatten().map { file -> tuple(file.name.substring(13,22), file) }.groupTuple().set { local_lr_corr_chunks_prep_bin }

	// Perform local correction for each chunk
	local_lr_corr_chunks_fq = localCorrection(local_lr_corr_chunks_prep_bin, lr_filt_map2mixhap_asm.bam, sr_filt_mixhap_asm_merge_bam).collect()
	// Merge the corrected chunks
	local_lr_corr_merge_fq = mergeLocalCorrection(local_lr_corr_chunks_fq, lr_filt_map2mixhap_asm.bam)

	// Haplotype-resolved assembly of the locally corrected reads
	hapres_asm = hapResAsm(local_lr_corr_merge_fq)
	// Basic haplotig filtering based on min coverage and length
	hapres_filt_asm = hapResAsm_filter(local_lr_corr_merge_fq, hapres_asm)
	...
	triobin = hapResAsmPolish_trioBin(	filt_mixhap_asm,
						hapres_filt_polish_asm, hapres_filt_polish_asm_withAlts,
						hapres_filt_polish_asm_phasing_filt_phased2.hap_bam, hapres_filt_polish_asm_phasing_filt_phased2.hap_vcf,
						parents_sr_dbg_bin.paternal, parents_sr_dbg_bin.maternal)
	...
	// Combine each [sr.bam, sr.bam.bai] chunk with the created dual assembly
	sr_chunks_fq.combine(dual_asm_polishSNP).set { sr_chunks_dual_asm }

	// Map the splitted Illumina chunks to the dual assembly
	sr_dual_asm_chunks_bam = mapPairedIllumina_polish(sr_chunks_dual_asm, false)
